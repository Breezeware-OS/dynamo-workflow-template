...
import {takeVitalsFormJson} from './VitalForm';
import {
  fetchTakeVitalsData,
...
export default function TakeVitals() {
  const navigate = useNavigate();
  const dispatch = useDispatch();
  const {id} = useParams();

  const takeVitalsFormData = useSelector(
    state => state.reviewForm.vitalFormData,
  );

  useEffect(() => {
    dispatch(fetchTakeVitalsData(id));
  }, []);

  const form = takeVitalsFormData?.taskForm?.formSchemaAndDataJson || '{}';
  ...
  const formik = useFormik({
    ...
    onSubmit: async values => {
      ...
      const updatedData = {
        ...takeVitalsFormData,
        id: takeVitalsFormData?.applicationId,
      };
      ...
      dispatch(saveTakeVitalsData(postData)).then(() => {
        navigate('/');
      });
    },
    ...
    <Grid container padding={1} style={{marginTop: '8px'}}>
      ...
      <Grid item xs={12} marginBottom="8px">
        <Breadcrumbs separator=">">
          ...
          <Text style={{color: ' #aaaaaa'}}>
            {' '}
            {`${takeVitalsFormData?.contextData?.intakeForm?.entityProperties['General Information'][0]?.value} 
            ${takeVitalsFormData?.contextData?.intakeForm?.entityProperties['General Information'][1]?.value}`}{' '}
          </Text>
        </Breadcrumbs>
      </Grid>

      <Grid item xs={12} marginBottom="8px">
        <Text type="h1">Take Patient Vitals</Text>
      </Grid>
      ...
      <Grid
        ...
        <RenderPatientDetails
          patientInfo={
            takeVitalsFormData?.contextData?.intakeForm?.entityProperties[
              'General Information'
            ]
          }
        />
