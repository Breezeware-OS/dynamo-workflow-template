...
export default function RenderNurseNotes({patientData, viewData}) {
  let formData;
  if (patientData) {
    formData = patientData?.contextData?.patientReview?.entityProperties || {};
  }

  if (viewData) {
    formData = viewData?.patientReview || {};
  }
  return (
